...
def process_server(responder, port):
    with http.server.HTTPServer(("10.92.80.10", port), responder) as server:
...
def responder(request):
    """
    Fixture that starts a local http server in a separate process on 10.92.80.10
...
def test_server_and_default_headers(responder, read_method, parquet_engine):
    ...
    if parquet_engine is None:
        df_http = read_method(f"http://10.92.80.10:{responder}")
    else:
        df_http = read_method(f"http://10.92.80.10:{responder}", engine=parquet_engine)
